...
function renderTabBar() {
  ...
  const tabsHTML = Object.keys(TAB_ROUTES).map(tabKey => {
...
function navigate(route, params = {}) {
  ...
  const isTabRoute = ['home', 'learn', 'data', 'progress', 'more'].includes(route);
    ...
    <div class="card vault-widget" id="vaultWidget">
      ...
      <div class="vault-metrics">
        ...
        <div class="vault-metric">
          ...
          <strong style="font-size: 18px; color: ${parseFloat(latestPnl) >= 0 ? 'var(--color-success)' : 'var(--color-error)'};">$
            ${formatNumber(latestPnl)}</strong>
        </div>
      </div>
      
      <div style="margin-top: var(--space-lg);">
        <h4 style="font-size: 16px; margin-bottom: var(--space-sm);">Открытые позиции (${openPositions.length})</h4>
        ${openPositions.length === 0 
          ? '<p class="caption">Нет открытых позиций</p>'
          : `<div class="vault-positions">
              ${openPositions.map(ap => {
                ...
                return `
                  <div class="vault-position">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                      ...
                      <span style="color: ${isProfit ? 'var(--color-success)' : 'var(--color-error)'}; font-weight: 600;">
                        ${isProfit ? '+' : ''}${formatCurrency(pnl)}
                      </span>
                    </div>
                    <div style="display: flex; gap: var(--space-md); margin-top: var(--space-xs);">
                      <span class="caption">Size: ${pos.szi}</span>
                      <span class="caption">Entry: $${formatNumber(pos.entryPx)}</span>
                      <span class="caption">Leverage: ${pos.leverage?.value || 'N/A'}x</span>
                    </div>
                  </div>
                `;
              }).join('')}
      ...
      <div style="margin-top: var(--space-md);">
        <a href="https://app.hyperliquid.xyz/vaults/${HYPERLIQUID_API.vaultAddress}" 
           class="btn btn-primary" 
           data-action="openLink" 
           data-url="https://app.hyperliquid.xyz/vaults/${HYPERLIQUID_API.vaultAddress}"
    ...
    <section class="content-section">
      ...
      <div class="nav-grid-compact">
        ...
        <div class="nav-card" data-action="navigate" data-route="achievements">
          ...
          <p class="caption" style="margin: var(--space-xs) 0 0 0; font-size: 12px;">${
    AppState.userData.unlockedBadges.length
  } разблокировано</p>
        </div>
      </div>
    </section>
...
function renderAchievements(container) {
    const allBadges = [
      ...
      { id: "collapsible_master", icon: "📂", name: "Организатор", desc: "Использование аккордеонов", condition: "Раскройте 20 аккордеонов", category: "education" },
      { id: "statistics_buff", icon: "📊", name: "Статистик", desc: "Изучение данных", condition: "Изучите все статистические данные", category: "education" },
      { id: "diversification_pro", icon: "🎲", name: "Профи диверсификации", desc: "Изучение стратегий", condition: "Изучите тему диверсификации", category: "education" },
      ...
      { id: "home_returner", icon: "🏠", name: "Домосед", desc: "Возврат на главную", condition: "Вернитесь на главную страницу 10 раз", category: "activity" },
      { id: "explorer", icon: "🧭", name: "Путешественник", desc: "Посещение разделов", condition: "Посетите все разделы приложения", category: "activity" },
      { id: "navigator", icon: "🗺️", name: "Навигатор", desc: "Частое перемещение", condition: "Совершите 50 переходов между разделами", category: "activity" },
      { id: "theme_switcher", icon: "🌓", name: "Переключатель", desc: "Смена темы", condition: "Переключите тему 5 раз", category: "activity" },
      { id: "dark_mode_fan", icon: "🌙", name: "Любитель темноты", desc: "Использование темной темы", condition: "Используйте темную тему 7 дней подряд", category: "activity" },
      { id: "light_mode_fan", icon: "☀️", name: "Любитель света", desc: "Использование светлой темы", condition: "Используйте светлую тему 7 дней подряд", category: "activity" },
      { id: "settings_explorer", icon: "⚙️", name: "Настройщик", desc: "Изучение настроек", condition: "Посетите раздел настроек", category: "activity" },

      // 📄 Документы и инструкции (6 достижений)
      { id: "doc_reader", icon: "📄", name: "Документовед", desc: "Чтение документов", condition: "Откройте первый документ", category: "documents" },
      { id: "all_docs", icon: "📋", name: "Юрист", desc: "Все документы", condition: "Откройте все официальные документы", category: "documents" },
      { id: "instruction_follower", icon: "📖", name: "Исполнитель", desc: "Чтение инструкций", condition: "Откройте 5 инструкций", category: "documents" },
      { id: "vault_explorer", icon: "🏦", name: "Хранитель", desc: "Изучение Vault", condition: "Откройте инструкции по Vault", category: "documents" },
      { id: "safepal_user", icon: "💳", name: "Владелец кошелька", desc: "Настройка SafePal", condition: "Изучите инструкции SafePal", category: "documents" },
      { id: "faq_master", icon: "❓", name: "Знаток FAQ", desc: "Изучение FAQ", condition: "Откройте раздел FAQ", category: "documents" },

      // 🎪 Эфиры и анонсы (5 достижений)
      { id: "broadcast_viewer", icon: "📡", name: "Зритель эфиров", desc: "Просмотр эфира", condition: "Откройте запись эфира", category: "broadcasts" },
      { id: "live_participant", icon: "🔴", name: "Участник LIVE", desc: "Участие в прямом эфире", condition: "Посетите раздел эфиров до начала трансляции", category: "broadcasts" },
      { id: "announcement_reader", icon: "📢", name: "В курсе событий", desc: "Чтение анонсов", condition: "Откройте раздел анонсов", category: "broadcasts" },
      { id: "question_asker", icon: "✍️", name: "Любознательный", desc: "Вопрос к эфиру", condition: "Задайте вопрос к предстоящему эфиру", category: "broadcasts" },
      { id: "regular_viewer", icon: "🎥", name: "Постоянный зритель", desc: "Просмотр эфиров", condition: "Посмотрите 5 записей эфиров", category: "broadcasts" },

      // 💬 Поддержка и взаимодействие (4 достижения)
      { id: "support_contact", icon: "💬", name: "Общительный", desc: "Обращение в поддержку", condition: "Откройте раздел поддержки", category: "support" },
      { id: "feedback_giver", icon: "📬", name: "Советчик", desc: "Обратная связь", condition: "Оставьте обратную связь", category: "support" },
      { id: "dashboard_user", icon: "📊", name: "Аналитик", desc: "Использование дашборда", condition: "Откройте дашборд инвестора", category: "support" },
      { id: "share_master", icon: "📤", name: "Распространитель", desc: "Поделиться результатами", condition: "Поделитесь результатом квиза", category: "support" },

      // 🏅 Специальные (10 достижений)
      { id: "first_login_morning", icon: "🌄", name: "Рассвет", desc: "Первый вход утром", condition: "Первый вход в период 6:00-9:00", category: "special" },
      { id: "weekend_enthusiast", icon: "🎉", name: "Выходной энтузиаст", desc: "Активность в выходные", condition: "Будьте активны 4 выходных подряд", category: "special" },
      { id: "new_year", icon: "🎊", name: "С Новым Годом!", desc: "Новогодний визит", condition: "Зайдите 1 января", category: "special" },
      { id: "christmas", icon: "🎄", name: "Рождественский дух", desc: "Рождественский визит", condition: "Зайдите 25 декабря или 7 января", category: "special" },
      { id: "valentine", icon: "💝", name: "День влюбленных", desc: "Романтичный визит", condition: "Зайдите 14 февраля", category: "special" },
      { id: "beta_tester", icon: "🧪", name: "Бета-тестер", desc: "Ранний пользователь", condition: "Используйте приложение в первый месяц", category: "special" },
      { id: "bug_reporter", icon: "🐛", name: "Охотник за багами", desc: "Сообщение об ошибке", condition: "Сообщите об ошибке", category: "special" },
      { id: "lucky_number", icon: "🍀", name: "Счастливчик", desc: "Счастливое число", condition: "Наберите ровно 77% в квизе", category: "special" },
      { id: "midnight_scholar", icon: "🌃", name: "Полуночник", desc: "Обучение ночью", condition: "Пройдите квиз между 00:00 и 03:00", category: "special" },
      { id: "completionist", icon: "💯", name: "Завершитель", desc: "Всё выполнено", condition: "Разблокируйте 40 достижений", category: "special" },
    ];
    ...
    const unlockedCount = allBadges.filter(b => AppState.userData.unlockedBadges.includes(b.id)).length;
...
function renderProfile(container) {
    ...
    container.innerHTML = `<h1>Профиль</h1><div class="card mb-lg"><h3>Статистика</h3><p>Серия: <strong>${
      ...
      userData.streak
    )}</strong></p><p>Завершено квизов: <strong>${
      userData.completedQuizzes.length
                ...
                ${group.items
                  .map((item) => {
                    ...
                    <div class="list-item ${
                      ...
                      !isPlaceholder ? `data-action="openLink" data-url="${item.url}"` : ""
                    }>
                      <div style="flex: 1;">
...
function renderAnnouncements(container) {
    ...
    <div class="card-grid">
      ${announcements
        .map((announcement) => {
          const date = announcement.date ? new Date(announcement.date) : new Date();
          ...
          <div class="card ${isPast ? "opacity-50" : ""}">
            <div style="display: flex; align-items: start; gap: var(--space-md); margin-bottom: var(--space-md);">
              <div style="font-size: 40px; line-height: 1;">${announcement.icon || '📢'}</div>
              <div style="flex: 1;">
                <div class="caption" style="color: var(--color-primary); font-weight: 600; margin-bottom: var(--space-xs);">
                  ${dateStr}, ${announcement.time || '—'} ${announcement.timezone || 'МСК'}
                </div>
                <h3 style="margin-bottom: var(--space-sm);">${announcement.title || 'Без названия'}</h3>
              </div>
            </div>
          ...
          .map((broadcast) => {
            ...
            <div class="card" style="border: 2px solid var(--color-primary);">
              <div class="preview-label" style="color: var(--color-primary);">
                ${broadcast.day || 'Дата'}, ${dateStr} • ${broadcast.time || '—'} ${broadcast.timezone || 'МСК'}
              </div>
              <h3>${broadcast.title || 'Без названия'}</h3>
              <p class="caption" style="margin-bottom: ${broadcast.recordUrl ? 'var(--space-md)' : '0'};">${broadcast.description || ''}</p>
              ${
                broadcast.recordUrl
        ...
        .map((broadcast) => {
          ...
          <div class="list-item">
            <div style="flex: 1;">
              ...
              <p class="caption" style="margin: var(--space-xs) 0 0 0;">${broadcast.description || ''}</p>
            </div>
            ${
              broadcast.recordUrl
    ...
    <div class="card-grid">
      ${data.channels
        ...
        <div class="card card-interactive" ${
          !isPlaceholder ? `data-action="openLink" data-url="${channel.url}"` : ""
        }>
...
function renderDocuments(container) {
    ...
    <div class="card-grid">
      ${documents
        .map(
          (doc) => `
        <div class="card card-interactive" data-action="openLink" data-url="${doc.url || '#'}">
          <div style="display: flex; align-items: start; gap: var(--space-md);">
            <div style="font-size: 40px; line-height: 1;">${doc.icon || '📄'}</div>
            <div style="flex: 1;">
              <h3 style="margin-bottom: var(--space-sm);">${doc.title || 'Без названия'}</h3>
...
async function applyDashboardFilters() {
  ...
  DashboardState.filteredData = DashboardState.data.filter(position => {
    ...
    if (DashboardState.filters.year && year !== parseInt(DashboardState.filters.year)) {
      return false;
    }
    
    if (DashboardState.filters.month && month !== parseInt(DashboardState.filters.month)) {
      return false;
    }
    
    if (DashboardState.filters.coin && position.coin !== DashboardState.filters.coin) {
      return false;
    }
    
    if (DashboardState.filters.direction && position.direction !== DashboardState.filters.direction) {
      return false;
    }
    
    if (DashboardState.filters.leverage && position.leverage !== DashboardState.filters.leverage) {
      return false;
    }
...
function populateDashboardFilters() {
  ...
  // Years
  const years = [...new Set(DashboardState.data.map(p => new Date(p.startDate).getFullYear()))].sort((a, b) => b - a);
  ...
  if (monthSelect) {
    ...
    monthSelect.innerHTML = '<option value="">Все месяцы</option>' +
      Object.entries(months).map(([num, name]) => `<option value="${num}">${name}</option>`).join('');
  ...
  if (coinSelect) {
    ...
    coinSelect.innerHTML = '<option value="">Все активы</option>' +
  ...
  // Leverage
  const leverages = [...new Set(DashboardState.data.map(p => p.leverage))].sort();
...
function animateNumber(element, start, end, duration = 1000, isCurrency = true) {
  if (!element) return;
  ...
  const timer = setInterval(() => {
    ...
    element.textContent = isCurrency ? formatCurrency(roundedValue) : roundedValue.toLocaleString('ru-RU');
  }, 16);
}
  ...
  return `
    <div class="dashboard-stats">
      <div class="stat-card">
        ...
        <div class="stat-value ${stats.totalPnl >= 0 ? 'positive' : 'negative'}" data-animate="${stats.totalPnl}">
          ${formatCurrency(0)}
        </div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-value ${stats.grossPnl >= 0 ? 'positive' : 'negative'}" data-animate="${stats.grossPnl}">
          ${formatCurrency(0)}
        </div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" data-animate="${stats.winningPositions}" data-type="number">0</div>
        <div class="stat-subtitle">${stats.winRate.toFixed(1)}% win rate</div>
      </div>
      
      <div class="stat-card">
        <div class="stat-title">Всего Позиций</div>
        <div class="stat-value" data-animate="${stats.totalPositions}" data-type="number">0</div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-value ${stats.totalFunding >= 0 ? 'positive' : 'negative'}" data-animate="${stats.totalFunding}">
          ${formatCurrency(0)}
        </div>
  ...
  return `
    <div class="bar-chart">
      ${data.map(item => {
        ...
        const tooltip = `${item.coin}: ${formatCurrency(item.totalPnl)} • ${item.positions} позиций • ${item.wins} успешных`;
        
        return `
          <div class="bar-item" data-tooltip="${tooltip}">
            <span class="bar-label" style="color: ${coinColor}">${item.coin}</span>
            ...
            <span class="bar-value ${item.totalPnl >= 0 ? 'positive' : 'negative'}">
      ...
      <div class="pie-legend">
        ${slices.map(slice => `
          <div class="legend-item">
            <div class="legend-color" style="--coin-color: ${slice.color}"></div>
    ...
    <div class="position-card">
      <div class="position-header">
        <div class="position-coin">
          <span class="position-coin-symbol" style="--coin-color: ${coinColor}">${position.coin}</span>
      ...
      <div class="position-details">
        ...
        <div class="detail-item">
          ...
          <div class="detail-value">${formatCurrency(position.volumeWithLeverage)}</div>
        </div>
        <div class="detail-item">
        ...
        <div class="detail-item">
          ...
          <div class="detail-value ${position.funding >= 0 ? 'positive' : 'negative'}">
      ...
      <div class="position-pnl">
        ...
        <span class="position-pnl-value ${position.pnl >= 0 ? 'positive' : 'negative'}">
...
async function renderDashboard(container) {
  ...
  const sortedByPnl = [...groupedData].sort((a, b) => Math.abs(b.totalPnl) - Math.abs(a.totalPnl));
